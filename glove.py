...
def vectorise_answers():
	...
	answers = answers[:10]
	answers_words = np.zeros((len(answers), largest_num_of_words_in_answer, largest_num_of_words_any_paragraph+1))
	answer_num = 0
...
def get_answer_dictionary(answer_num, paragraphs_str, paragraph_question_mapping, largest_num_of_words_any_paragraph):
	...
	for sentence in sentences:
		words = sentence.split(' ')
		for word in words:
			answer_one_hot = [0 for i in range(largest_num_of_words_any_paragraph+1)]	
			characters = list(word)
